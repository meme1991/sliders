...
class Replace
{
	...
	private static function replaceSyntaxItem(&$string, $item, $items, $first = 0)
	{
		...
		if ( ! in_array(self::$context, ['com_search.search', 'com_finder.indexer']))
		{
			...
			$html[] = '<div class="card-header accordion-heading panel-heading"' . $heading_attributes . '>';
			...
			$html[] = '<div class="card-body accordion-inner panel-body">';
		}
	...
	{
		$class = ['card accordion-group panel rl_sliders-group nn_sliders-group'];
	...
	private static function getSliderTitle($item, $items)
	{
		...
		return
			'<a href="#' . $item->id . '" title="' . htmlspecialchars($item->title) . '" class="text-primary ' . $class . '"' . $onclick . $link_attributes . '>'
